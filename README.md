...
- **Calibration 提升（2026-09-10）**：Batch 01（10 Critical）+ Batch 02–07
  （52 Critical）——42 个候选经确定性验证 + 严格门后提升为 AUTO_ACCEPT 并并入
  canonical（含 2026-09-10 救援：16 个证据行不足的隔离候选补第二真实章节出处后
  提升）；`event_evidence` 26 → 130 条（term-level 出处行）；10 个候选因
  source_reference 暂无第二章节级出处保留为 QUARANTINE_MEDIUM（候选与来源留存）。
