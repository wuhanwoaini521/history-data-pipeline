# -*- coding: utf-8 -*-
"""China History Backbone V1 — Batch 7（清→晚清→辛亥革命）Writer。

写入 events/qing（全 6 阶段）、reviews、candidates（batch7_<phase>_candidates.yml）。
"""
...
from backbone_batch7_tail_data3 import PHASES, all_events  # noqa: E402
...
REVIEWED_BY = "china-history-backbone-v1-curator (batch7)"
FIELD_ORDER = [
...
PHASE_META = {
    "QING_ENTRY": ("Phase 1 清入关与统一", "qing"),
    "QING_HIGH": ("Phase 2 清前中期", "qing"),
    "OPIUM_TAIPING": ("Phase 3 鸦片战争与太平天国", "qing"),
    "WESTERN_SELF": ("Phase 4 洋务与边疆", "qing"),
    "SINO_JAPAN_BOXER": ("Phase 5 甲午与庚子", "qing"),
    "REVOLUTION": ("Phase 6 清末新政与辛亥革命", "qing"),
}
...
def _event_yaml(ev: dict) -> str:
    ...
    lines = [
        "# China History Backbone V1 · Batch 7（清→晚清→辛亥革命）",
...
def _review_json(ev: dict) -> dict:
    note = (
        "Batch 7（清→晚清→辛亥革命）人工审核：确认事件必要性、时期归属、时间与重要性；"
...
def _candidate_yaml(phase: str, events: list) -> str:
    ...
    doc = {
        ...
        "phase": label,
        "compiled_from": "AI 依据《清史稿》《清实录》《筹办夷务始末》《太平天国文书汇编》"
                         "《中法越南交涉资料》《义和团档案史料》《辛亥革命》资料丛刊及现代清史/"
                         "近代史研究整理候选；候选进入 review，accepted 后才写入 curated events/",
        "flow": "candidate → data/reviews/accepted/<event_id>.review.json → curated events/",
        "reused_events": ["event-houjin-jianguo（1616 后金建立）", "event-lizicheng-gong-beijing（1644 明亡）"],
        "candidates": [
            {
    ...
    return "# China History Backbone V1 · Batch 7 候选（Candidate）\n" + buf.getvalue()
    ...
    for phase in phases:
        ...
        for ev in PHASES[phase]:
            ...
            if not review.exists():
                ...
                written_reviews += 1
        if not args.phase:
            (CANDIDATES_DIR / f"batch7_{phase.lower()}_candidates.yml").write_text(
