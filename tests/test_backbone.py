...
def test_backbone_loads(backbone):
    ...
    # 基础 26 + Batch1-5（392）+ Batch 6（82）+ Batch 7（66）+ Batch 8（52：7+8+6+7+5+19）
...
def test_resolution_linked_places(resolution):
    ...
    # Major Batch 02 后地点关联 308（3 linked + 305 needs_linking）
    assert resolution.places["needs_linking"] == 305
...
def test_resolution_no_broken(resolution):
    ...
    # Major Batch 02 后 evidence 1177 条中 14 条 needs_linking（语料缺著作）不带锚 → 1163 条降级 pending。
    assert resolution.evidences["pending_knowledge"] == 1163
